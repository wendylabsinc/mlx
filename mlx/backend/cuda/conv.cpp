...
void FusedConvBiasActivation::eval_gpu(
  ...
  if (auto it = fused_conv_cache().find(cache_key);
    ...
    CHECK_CUDNN_FE_ERROR(register_and_encode(*graph, in, wt, out));
    return;
  }
  ...
  auto graph = build_conv_graph(
      ...
      /* apply_silu */ true);
  if (!graph) {
    // ponytail: standard YOLOv9 convs always build a conv+bias+swish graph on
    // cuDNN 9.x; if a future shape doesn't, add a decomposed fallback here.
  ...
  // Autotune (no-op unless MLX_CUDNN_AUTOTUNE) before capture, once per shape.
  CHECK_CUDNN_FE_ERROR(graph->autotune_plans(encoder, variant_pack));
  encoder.set_input_array(x);
  encoder.set_input_array(w);
  encoder.set_input_array(bias);
